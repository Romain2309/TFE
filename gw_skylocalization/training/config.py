...
class TrainingConfig:
    ...
    loss_type: str = 'angular'  # 'angular', 'cosine' (for regressor/multitask)
    tau_weight: float = 0.1  # Weight for time delay loss (multitask only)
    label_smoothing: float = 0.0  # For classifier/probmap

    optimizer: str = 'adamw'  # 'adam', 'adamw', 'sgd'
    momentum: float = 0.9  # For SGD

    scheduler_type: str = 'plateau'  # 'plateau', 'cosine', 'step', 'onecycle', None
    scheduler_patience: int = 5  # For ReduceLROnPlateau
    scheduler_factor: float = 0.5  # For ReduceLROnPlateau/StepLR
    scheduler_step_size: int = 10  # For StepLR

    early_stopping: bool = True
    patience: int = 15
    min_delta: float = 0.0  # Minimum improvement to count as progress

    save_best_only: bool = True
    save_last: bool = True
    monitor_metric: str = 'val_loss'  # Metric to monitor for best model

    num_workers: int = 4
    pin_memory: bool = True
    cache_train: bool = False  # Cache training data in memory (small datasets only)
    ...
    mixed_precision: bool = False  # Use automatic mixed precision (AMP)
...
if __name__ == "__main__":
    ...
    print(f"Original experiment name: {custom_config.experiment_name}")
    ...
    print(f"Bandpass: {config.bandpass}")
