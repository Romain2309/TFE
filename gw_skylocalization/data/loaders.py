...
def create_dataloaders(
    ...
    if cache_train:
        if verbose:
            print(f"Creating cached training dataset ({n_train} events)...")
            print("  This will take time on first epoch, but subsequent epochs will be much faster!")
...
if __name__ == "__main__":
    ...
    for key, val in sample.items():
        print(f"  {key}: {val.shape if hasattr(val, 'shape') else type(val)} {val.dtype}")

    print("\n2. Testing create_dataloaders...")
