...
```python
def create_model(model_type, nside, **kwargs):
    if model_type == 'regressor':
        return Regressor(nside, **kwargs)
    elif model_type == 'classifier':
        return Classifier(nside, **kwargs)
    # ...
```

### Builder Pattern
Used for data loaders:
```python
def create_dataloaders(dataset_path, batch_size, **kwargs):
    dataset = GWDataset(dataset_path, **kwargs)
...
class RegressionLossStrategy(LossStrategy):
    def compute(self, predictions, targets):
        return F.mse_loss(predictions, targets)
```
